...
if($_POST['submit']) {
    $authsql = "SELECT * FROM subjects WHERE id = " . $validsubject
            . " AND owner_id = " . $_SESSION['SESS_USERID'] . ";";
    $authresult = mysql_query($authsql);
    $authnumrows = mysql_num_rows($authresult);
    if($authnumrows == 1) {
        $qsql = "INSERT INTO questions(topic_id, question, answer,
addedby_id, dateadded, active) VALUES("
                . $_POST['topic']
...
addedby_id, dateadded, active) VALUES("
        ...
        echo "Your question requires moderator approval before it is
posted.";
    }
}
else {
require("header.php");
$subsql = "SELECT * FROM subjects WHERE id = "
        . $validsubject . ";";
$subq = mysql_query($subsql);
$subrow = mysql_fetch_assoc($subq);
$toplistsql = "SELECT * FROM topics WHERE subject_id = "
        . $validsubject . " ORDER BY name ASC;";
...
if($notopics == TRUE) {
    echo "No topics have been created. Click
<a href='addtopic.php'>here</a> to create one!";
}
else {
    echo "<p>";
    echo "<form action='addquestion.php?subject="
            . $validsubject . "' method='POST'>";
    echo "<table cellpadding=5>";
    echo "<tr>";
    echo "<td>Subject</td>";
    echo "<td><strong>" . $subrow['subject']
            . "</strong></td>";
    echo "</tr>";
    echo "<tr>";
    echo "<td>Topic</td>";
    ...
    if(!$validtopic) {
        ...
        while($toplistrow =
                mysql_fetch_assoc($toplistresult)) {
            echo "<option value='"
                    . $toplistrow['id'] . "'>" . $toplistrow['name']
                    . "</option>";
        }
        echo "</select>";
    }
    else {
        $topsql = "SELECT * FROM topics WHERE id = "
                . $validtopic . ";";
    ...
    echo "<tr>";
    echo "<td>Answer</td>";
    echo "<td><textarea name='answer' rows=10
cols=50></textarea></td>";
    echo "</tr>";
    echo "<tr>";
